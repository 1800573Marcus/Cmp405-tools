...
void ToolMain::Tick(MSG *msg)
{
	...
	if (m_toolInputCommands.clicked)
	{
		
	}
	if (m_toolInputCommands.dblclicked) {
		m_selectedObject = m_d3dRenderer.MousePicking();
		m_toolInputCommands.dblclicked = false;
	}
...
void ToolMain::UpdateInput(MSG * msg)
{
	...
	case WM_MOUSEMOVE:
		//update mouse position for camera
		 yPosmove = GET_Y_LPARAM(msg->lParam);
		 xPosmove = GET_X_LPARAM(msg->lParam);
	...
	case WM_LBUTTONUP:
		//updates that mouse is no longer clicked
		m_toolInputCommands.mouseup = true;
		m_toolInputCommands.clicked = false;
		m_toolInputCommands.dblclicked = false;


		break;
	case WM_LBUTTONDOWN:
		//updates mouse is clicked
		m_toolInputCommands.mouseup = false;

		m_toolInputCommands.clicked = true;
		break;

	case WM_LBUTTONDBLCLK:
		//update when mouse is double clicked
		m_toolInputCommands.dblclicked = true;
	...
	//object manipulation
	if (m_keyArray['I'])// move object positively along the X axis
	{
		m_toolInputCommands.I = true;
	}
	else m_toolInputCommands.I = false;
	if (m_keyArray['K'])// move object negatively along the X axis
	{
		m_toolInputCommands.K = true;
	}
	else m_toolInputCommands.K = false;
	if (m_keyArray['J'])// move object positively along the Y axis
	{
		m_toolInputCommands.J = true;
	}
	else m_toolInputCommands.J = false;

	if (m_keyArray['L'])// move object negatively along the Y axis
	{
		m_toolInputCommands.L = true;
	}

	else m_toolInputCommands.L = false;
	if (m_keyArray['U'])// move object positively along the Z axis
	{
		m_toolInputCommands.U = true;
	}
	else m_toolInputCommands.U = false;
	if (m_keyArray['O'])// move object negatively along the Z axis
	{
		m_toolInputCommands.O = true;
	}
	else m_toolInputCommands.O = false;
	if (m_keyArray['M'])// Increase scale of selected object
	{
		m_toolInputCommands.scaleup = true;
	}
	else m_toolInputCommands.scaleup = false;
	if (m_keyArray['N'])// Decrease scale of selected object
	{
		m_toolInputCommands.scaledown = true;
	}
	else m_toolInputCommands.scaledown = false;
	if (m_keyArray['1'])// set camera to cam 1
	{
		m_toolInputCommands.cam1 = true;
	}
	else m_toolInputCommands.cam1 = false;
	if (m_keyArray['2'])// set camera to cam 2
	{
		m_toolInputCommands.cam2 = true;
	}
	else m_toolInputCommands.cam2 = false;
	if (m_keyArray['3'])// set camera to cam 3
	{
		m_toolInputCommands.cam3 = true;
	}
	else m_toolInputCommands.cam3 = false;
	if (m_keyArray['F'])// set camera to cam 3
	{
		m_toolInputCommands.wireframe = true;
	}
